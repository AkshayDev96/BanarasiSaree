...
import {Update} from '../../actions/subcategory'
...
const UpdateSubCategory = ({subcategory}) => {
    ...
    const [category,setCategory] = React.useState(subcategory.category)
   ...
   const onDrop = (acceptedFile, rejectedFiles) => {
    ...
    const validCheck =()=>(((image!==undefined && image!=='')||(subcategory.image!==undefined && subcategory.image!=='')) 
    && (category!==undefined && category!=='') && (name!==undefined && name.trim()!==''))

    const submit=(e)=>{
        e.preventDefault()
        if(validCheck() && subcategory._id){
            let imagePath=""
            if(image){
                imagePath = image.url
            }else{
            ...
            <div className="row">
                    ...
                    <h3>Create subcategory</h3>
                        ...
                        <div className="form-group">
                            <select onChange={(e)=>setCategory(e.target.value)} className="form-select form-select-md form-control" value={category} aria-label=".form-select-sm example">
                        ...
                        <div className="form-group">
                         ...
                         {subcategory.image?(
                            <div className="form-group">
                                <ImageView data={subcategory.image}/>
                              </div>
                         ):null}
