...
import {MakeCategory} from '../../actions/category'
...
const CreateCategory = () => {
   ...
   const onDrop = (acceptedFile, rejectedFiles) => {
    ...
    const validCheck =()=>((image!==undefined && image!=='') && (name!==undefined && name.trim()!==''))
    ...
    const submit=(e)=>{
        ...
        if(validCheck()){
            ...
            MakeCategory(request).then((res)=>{
                if(res.data.success && res.data.message){
                    alert(res.data.message)
                    router.replace('/admin/category/view')
